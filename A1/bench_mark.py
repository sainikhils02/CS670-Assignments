#!/usr/bin/env python3
"""Benchmark runtime per query for Assignment 1 updates.

The script automates the following loop:
1. Regenerate shares/queries with a chosen (num_users, num_items, num_features, num_queries).
2. Run only the MPC parties (p2, p1, p0) under docker compose and time the run.
3. Store the average wall-clock time per query (equivalently one user/item update) and emit comparison plots for
...
def run_command(args: List[str], *, dry_run: bool, check: bool = True) -> None:
    """Run a subprocess inside A1_ROOT, printing the command beforehand."""

    printable = " ".join(args)
    print(f"[cmd] {printable}")
    if dry_run:
        return
    subprocess.run(args, cwd=A1_ROOT, check=check)
...
def benchmark_point(
    *,
    ...
    for rep in range(repetitions):
        ...
        regenerate_data(users, items, features, queries, dry_run=dry_run)
...
def plot_results(
    ...
    *,
    ...
    for label, points in grouped.items():
        ...
        ys = [pt[value_key] for pt in points_sorted]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Number of latent features per user/item (default: %(default)s)",
    )
    parser.add_argument(
        "--repetitions",
        type=int,
